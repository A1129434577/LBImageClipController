...
- (void)setMaskRect:(CGRect)maskRect animated:(BOOL)animated {
    ...
    [self removeAnimationForKey:@"LB_maskLayer_opacityAnimate"];
    if (animated) {
        ...
        animate.toValue = @(1.0);
        self.path = mPath;
        [self addAnimation:animate forKey:@"LB_maskLayer_opacityAnimate"];
    } else {
        self.path = mPath;
    }
}
...
/// 网格层
@interface LBGridLayer : CAShapeLayer
///网格区域 默认CGRectZero
...
@class LBResizeControl;
@protocol LBResizeControlDelegate <NSObject>
/// 开始
- (void)resizeConrolDidBeginResizing:(LBResizeControl *)resizeConrol;
- (void)resizeConrolDidResizing:(LBResizeControl *)resizeConrol;
- (void)resizeConrolDidEndResizing:(LBResizeControl *)resizeConrol;
@end
/// 网格四边和四角控制大小的透明视图
@interface LBResizeControl : UIView
@property (weak, nonatomic) id<LBResizeControlDelegate> delegate;
...
@end
@implementation LBResizeControl
...
// 网格四角和边的 可控范围
const CGFloat kLBControlWidth = 30.f;
//网格视图
@interface LBGridView ()<LBResizeControlDelegate>
@property (nonatomic, strong) LBGridLayer *gridLayer; //网格层
@property (nonatomic, strong) LBGridMaskLayer *gridMaskLayer; // 半透明遮罩层
@property (nonatomic, assign) CGRect initialRect; //高亮网格框的初始区域
//四个角
@property (nonatomic, strong) LBResizeControl *topLeftCornerView;
@property (nonatomic, strong) LBResizeControl *topRightCornerView;
@property (nonatomic, strong) LBResizeControl *bottomLeftCornerView;
@property (nonatomic, strong) LBResizeControl *bottomRightCornerView;
//四条边
@property (nonatomic, strong) LBResizeControl *topEdgeView;
@property (nonatomic, strong) LBResizeControl *leftEdgeView;
@property (nonatomic, strong) LBResizeControl *bottomEdgeView;
@property (nonatomic, strong) LBResizeControl *rightEdgeView;
@end

@implementation LBGridView
#pragma mark - Override
...
- (void)enableCornerViewUserInteraction:(UIView *)view {
    for (UIView *control in self.subviews) {
        if ([control isKindOfClass:[LBResizeControl class]]) {
            if (view) {
                if (control == view) {
...
- (LBResizeControl *)createResizeControl {
    LBResizeControl *control = [[LBResizeControl alloc] initWithFrame:(CGRect){CGPointZero, CGSizeMake(kLBControlWidth, kLBControlWidth)}];
...
#pragma mark - Getter
- (LBGridMaskLayer *)gridMaskLayer {
    if (!_gridMaskLayer) {
        _gridMaskLayer = [[LBGridMaskLayer alloc] init];
...
- (LBGridLayer *)gridLayer {
    if (!_gridLayer) {
        _gridLayer = [[LBGridLayer alloc] init];
...
//返回正在调整网格大小时的网格区域
- (CGRect)cropRectMakeWithResizeControlView:(LBResizeControl *)resizeControlView {
    CGRect rect = self.gridRect;
...
#pragma mark - LBResizeControlDelegate
- (void)resizeConrolDidBeginResizing:(LBResizeControl *)resizeConrol {
